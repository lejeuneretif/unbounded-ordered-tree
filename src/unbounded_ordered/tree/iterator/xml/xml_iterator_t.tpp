#ifndef UNBOUNDED_ORDERED_TREE_XML_ITERATOR_T_TPP
#define UNBOUNDED_ORDERED_TREE_XML_ITERATOR_T_TPP
...
namespace unbounded_ordered {
  ...
  class tree<T>::xml_iterator_t
      : public tree<T>::template bidirectional_iterator_t<T_value> {
  protected:
    xml_iterator_t(node<T>* root, node<T>* current, typename bidirectional_iterator_t<T_value>::Status status,
    ...
    inline void next() {
      switch(this->_status) {
        ...
        case bidirectional_iterator_t<T_value>::BIDIRECTIONAL_ITERATOR_MIDDLE: {
          std::pair<node<T>*, typename node<T>::NodeLastMovement> next = this->_current->xml_next(this->_last_movement);
    ...
    inline void previous() {
      switch(this->_status) {
        case bidirectional_iterator_t<T_value>::BIDIRECTIONAL_ITERATOR_BEGIN:
          ...
          break;
        case bidirectional_iterator_t<T_value>::BIDIRECTIONAL_ITERATOR_MIDDLE: {
          std::pair<node<T>*, typename node<T>::NodeLastMovement> previous = this->_current->xml_previous(this->_last_movement);
  ...
  public:
    xml_iterator_t() : bidirectional_iterator_t<T_value>() {}
  };
}

#endif // UNBOUNDED_ORDERED_TREE_XML_ITERATOR_T_TPP