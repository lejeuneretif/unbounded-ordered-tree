#ifndef UNBOUNDED_ORDERED_TREE_LEAF_ITERATOR_T_TPP
#define UNBOUNDED_ORDERED_TREE_LEAF_ITERATOR_T_TPP
...
namespace unbounded_ordered {
  ...
  class tree<T>::leaf_iterator_t
      : public tree<T>::template bidirectional_iterator_t<T_value> {
  protected:
    leaf_iterator_t(node<T>* root, node<T>* current,
    ...
    inline void next() {
      switch(this->_status) {
        ...
        case bidirectional_iterator_t<T_value>::BIDIRECTIONAL_ITERATOR_MIDDLE: {
          std::pair<node<T>*, typename node<T>::NodeLastMovement> next = this->_current->leaf_next();
    ...
    inline void previous() {
      switch(this->_status) {
        case bidirectional_iterator_t<T_value>::BIDIRECTIONAL_ITERATOR_BEGIN:
          ...
          break;
        case bidirectional_iterator_t<T_value>::BIDIRECTIONAL_ITERATOR_MIDDLE: {
          std::pair<node<T>*, typename node<T>::NodeLastMovement> previous = this->_current->leaf_previous();
  ...
  public:
    leaf_iterator_t() : bidirectional_iterator_t<T_value>() {}
  };
}

#endif // UNBOUNDED_ORDERED_TREE_LEAF_ITERATOR_T_TPP